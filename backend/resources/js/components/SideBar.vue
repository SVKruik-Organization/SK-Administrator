...
<template>
    <nav :class="[props.isOpen ? 'translate-x-0' : '-translate-x-full']"
        class="flex flex-col h-screen bg-theme overflow-hidden fixed inset-y-0 left-0 z-30 w-64 transform transition-transform duration-300 md:sticky md:w-3xs md:translate-x-0">
        <div ref="profileDropdownRef" class="relative p-2">
            <!-- Profile switcher button -->
            <button
                class="group flex items-center gap-2 cursor-pointer hover:bg-theme-dark rounded-4xl relative w-full p-2 h-14"
                type="button" :class="{ 'active bg-theme-dark rounded-b-none shadow-lg': isProfileSwitcherOpen }"
                @click="toggleProfileSwitcher()">
                <div class="relative">
                    <img src="/temp_pfp.jpg" class="aspect-square object-cover h-12 rounded-full">
                    <span :class="isProfileSwitcherOpen ? 'border-theme-dark' : 'border-theme'"
                        class="h-4 w-4 bg-emerald-600 rounded-full absolute -top-0.5 -right-0.5 border-2 group-hover:border-theme-dark"></span>
                </div>
                ...
                <menu v-if="auth.profiles.length > 1 && isProfileSwitcherOpen"
                    class="flex flex-col absolute top-14 right-0 rounded-b-xl shadow-lg bg-theme-dark p-2 z-10">
                    <button v-for="profile in inactiveProfiles" :key="profile.id"
                        class="flex flex-col p-2 rounded-lg hover:bg-theme text-left cursor-pointer" type="button"
        ...
        <section class="flex flex-col h-full overflow-y-auto overflow-x-hidden pl-4 pt-4 gap-8">
            ...
            <div class="flex flex-col gap-4">
                <menu>
                    <Link v-for="item in auth.top_module_items" :key="item.id" :href="item.url"
                        :class="{ 'bg-theme-dark': getLinkStatus(item.url) }"
                        class="flex items-center gap-2 h-10 rounded-full px-4 box-border-2 w-11/12 hover:bg-theme-dark"
                        @click="closeSidebar()">
            ...
            <!-- Modules -->
            <div class="flex flex-col gap-4 last-of-type:mb-8" v-for="module in activeProfile.modules" :key="module.id">
